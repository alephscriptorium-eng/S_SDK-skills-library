...
description: >-
  ...
  append-only con fallback pull, hilos con lectura cruzada acotada y git por
...
3. Operar el rol que toque (carril, hub o auditor) bajo §1–§11: anunciarse,
   esperar tick, una nota por turno, firmar.
4. Al cierre de ronda/sesión: bitácora en `RAMA(X)`, snapshot de `SALA` y
   sello en `CUADERNOS` (§10.5–§10.7).
5. Antes de cara pública del skill: `scripts/comprobar-ceguera.sh` →
   `ceguera: 0`.
